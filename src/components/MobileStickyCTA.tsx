...
export default function MobileStickyCTA() {
  ...
  const appointmentText = hasContactSection
    ? isCommercial
      ? "預約場勘"
      : isRepair
        ? "完整估價"
  ...
  const phoneButton = (
    <CTAButton
      ...
      className="flex-1 flex flex-col items-center justify-center bg-slate-800 text-white hover:bg-slate-700 py-3 rounded-xl font-bold border border-slate-700 text-sm sm:text-lg min-h-[64px] gap-1 transition-colors"
    >
  ...
  const appointmentButton = (
    <CTAButton
      ...
      className="flex-1 flex flex-col items-center justify-center bg-orange-500 hover:bg-orange-400 text-white py-3 rounded-xl font-bold text-sm sm:text-lg min-h-[64px] gap-1 shadow-md shadow-orange-950/20 transition-colors"
    >
