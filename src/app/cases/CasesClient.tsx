      ...
      <section className="relative py-16 overflow-hidden bg-slate-900/10 border-b border-slate-900">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-4 leading-relaxed max-w-2xl mx-auto">
            商用大系統涉及複雜的管路、電力配線與建物荷載。我們堅持實務查勘，不捏造省電能效或回本時間。
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {cases.map((item, index) => (
              <div 
                key={index}
                className="bg-slate-900/40 border border-slate-850 rounded-2xl overflow-hidden flex flex-col hover:border-slate-755 transition-all duration-300 shadow-md group"
              >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <span className="text-[10px] font-bold text-sky-455 uppercase tracking-widest block bg-sky-950/40 px-2 py-0.5 rounded border border-sky-900/25">
                        {item.type}
                      </span>
