    ...
    <main className="flex-1 flex flex-col pt-16">
      ...
      <article className="py-16 bg-slate-950 flex-1">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Core Conclusion */}
          <div className="bg-sky-950/15 border-l-4 border-sky-500 p-6 rounded-r-xl mb-10 text-slate-200 text-base sm:text-lg leading-[1.8]">
          ...
          <div className="space-y-10 text-slate-200 text-base sm:text-lg leading-[1.8]">
            
            {/* Section 1 */}
            <section className="space-y-4">
              <h2 className="text-2xl font-bold text-white border-b border-slate-900 pb-2">
                一、冷氣有霉味的常見原因
              </h2>
              <div className="bg-slate-900/20 p-5 rounded-xl border border-slate-850/60 mb-4">
                <strong className="text-sky-300 block mb-2">Q：冷氣有霉味、出風量變小該如何解決？</strong>
                <p className="text-slate-200">
                  答：冷氣異味與風量減弱主要由內部髒污引起。解決方法為：<strong>清洗防塵濾網</strong>（恢復基礎進風）、<strong>使用高壓水槍深層清洗室內機鰭片與水盤</strong>（清除黴菌與發霉生物膜）、<strong>清洗鼓風輪與出風口積垢</strong>（暢通出風道並消除風量不均異音），若排水管接到大樓廢水管且無存水彎，則需<strong>加裝存水彎阻隔沼氣與管道臭味逆流</strong>。
                </p>
              </div>
              <p>
              ...
              <ul className="list-disc pl-6 space-y-2.5">
                ...
                <li><strong>排水管異味回流</strong>：排水管直接接到大樓的廢水管、雨水管 or 水溝，且沒有設計存水彎，導致管道內的沼氣或廢水臭味直接吸回室內。</li>
            ...
            {/* 三、技師如何判定深層髒污：出風口風速與風阻量測 */}
            <section className="space-y-4">
              <h2 className="text-xl sm:text-2xl font-bold text-white border-l-4 border-sky-500 pl-3">三、技師如何判定深層髒污：出風口風速與風阻量測</h2>
              <p>
                冷氣的出風量與異味是判斷內部是否發霉卡塵最直接的物理指標。空調技師到府時會利用儀器量測出風風速。
              </p>
              <div className="bg-slate-900/50 p-6 rounded-2xl border border-sky-950/40 space-y-3">
                <span className="text-sky-400 font-bold text-lg block">★ 冷氣發霉風阻與出風風速指標：</span>
                <ul className="list-disc pl-6 space-y-2.5">
                  <li><strong>正常運轉（風速 4.5 m/s 以上）：</strong>壁掛式室內機在開啟強風運轉時，出風口的平均風速應達到 <strong>4.5 m/s ~ 6.0 m/s 以上</strong>，代表風道無阻塞、熱交換效率優良。</li>
                  <li><strong>嚴重卡垢（風速低於 2.5 m/s）：</strong>若風鼓或濾網嚴重卡垢阻礙氣流，風速會明顯下降至 2.5 m/s 以下，且伴隨風量忽大忽小、出風不均的波動聲。高風阻也容易造成水滴被冷風帶出產生「噴水」現象，需安排深層高壓清洗保養。</li>
                </ul>
              </div>
            </section>
            ...
            <section className="space-y-4 py-4">
              <h3 className="text-xl font-bold text-white">冷氣異常狀況自我排查表</h3>
              <div className="overflow-x-auto border border-slate-850 rounded-xl bg-slate-900/20">
                <table className="min-w-full divide-y divide-slate-850/60 text-sm sm:text-base">
                  <thead className="bg-slate-900 text-slate-200">
                    <tr>
                      <th className="px-4 py-4 text-left font-bold border-r border-slate-850/60">狀況</th>
                      <th className="px-4 py-4 text-left font-bold border-r border-slate-850/60">可能原因</th>
                      <th className="px-4 py-4 text-left font-bold">建議處理方向</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-850/60 text-slate-200">
                    <tr>
                      <td className="px-4 py-4 border-r border-slate-850/60 font-semibold text-white">有霉味</td>
                      <td className="px-4 py-4 border-r border-slate-850/60">蒸發器與水盤發霉、滋生黴菌與生物膜</td>
                      <td className="px-4 py-4">進行防霉深層高壓清洗</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-4 border-r border-slate-850/60 font-semibold text-white">風量變小</td>
                      <td className="px-4 py-4 border-r border-slate-850/60">鼓風輪卡滿棉絮灰塵、濾網阻塞</td>
                      <td className="px-4 py-4">清洗鼓風輪與防塵濾網</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-4 border-r border-slate-850/60 font-semibold text-white">出風不均</td>
                      <td className="px-4 py-4 border-r border-slate-850/60">風鼓部分葉片被髒污堵死、馬達轉速異常</td>
                      <td className="px-4 py-4">進行風鼓清洗，若馬達異常需安排檢修</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-4 border-r border-slate-850/60 font-semibold text-white">排水異味</td>
                      <td className="px-4 py-4 border-r border-slate-850/60">排水管連接大樓排水溝、未做存水彎</td>
                      <td className="px-4 py-4">進行水盤清洗，並評估排水管改道或加裝存水彎</td>
                    </tr>
                    <tr>
                      <td className="px-4 py-4 border-r border-slate-850/60 font-semibold text-white">清洗後仍異常</td>
                      <td className="px-4 py-4 border-r border-slate-850/60">零件故障（如電容或基板）、漏冷媒</td>
                      <td className="px-4 py-4">安排技師到府進行故障查檢與維修</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <section className="space-y-4">
              ...
              <p>
                若您的冷氣有異味但出風完全是常溫（主機沒在運轉）、清洗後過幾天依然滴水、或者出風口完全沒有風吹出，這可能代表不僅是髒污問題，而是線路、冷媒系統或馬達硬體有所磨損故障。此時建議由專業技師安排實地查檢與評估，找出核心病因後對症下藥。
              </p>
            </section>

            {/* CTAs inside article */}
            <div className="pt-6 text-center space-y-6">
              <h3 className="text-lg font-bold text-white">冷氣霉味風量小？請 LINE 傳照初步分析或預約清洗</h3>
              ...
              <div className="flex justify-center gap-6 text-base text-sky-400 font-semibold pt-2">
                <Link href="/services/ac-cleaning/odor-mold/" className="hover:underline py-2">冷氣霉味清洗服務 &rarr;</Link>
                <Link href="/services/ac-cleaning/" className="hover:underline py-2">變頻冷氣清洗服務 &rarr;</Link>
                <Link href="/faq/" className="hover:underline py-2">常見保養問題 &rarr;</Link>
              </div>
            </div>

            {/* FAQ Section */}
            <section className="space-y-6 pt-10 border-t border-slate-900">
              <h2 className="text-2xl font-bold text-white text-center mb-6">冷氣發霉與清洗常見問答 FAQ</h2>
              ...
              <div className="space-y-6 max-w-3xl mx-auto">
                ...
                <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-850">
                  ...
                  <p className="text-slate-200 text-base sm:text-lg leading-[1.8]">答：不一定。最常見的原因是風鼓與濾網卡滿灰塵與棉絮，阻擋了出風氣流，清洗後通常可恢復風速。但也有可能是室內風扇馬達軸承磨損導致轉速變慢、啟動電容老化衰退、或是控制基板訊號故障，需要到府進行現場診斷。</p>
                </div>
                <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-850">
                  <h3 className="font-bold text-white mb-2 text-lg">Q：可以只清室內機嗎？</h3>
                  <p className="text-slate-200 text-base sm:text-lg leading-[1.8]">答：可以。室內機是主要的冷風出口與熱交換器，冷氣發霉、有霉味、粉塵飛散或滴水問題大都與室內機有關。若預算有限，可優先清洗室內機；但若室外機鰭片堆積大量塵土落葉影響散熱，仍建議一併施作。</p>
                </div>
                <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-850">
                ...
                <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-850">
                  ...
                  <p className="text-slate-200 text-base sm:text-lg leading-[1.8]">答：若冷氣運轉功能正常且有冷風，但伴隨霉味、出風量變小或出風口滴水，通常只需進行「清洗保養」。若冷氣完全無法啟動、電源燈閃爍報錯誤代碼、只吹送風（壓縮機沒動，完全無冷度）、或機器運作時發出金屬撞擊等劇烈噪音，則屬於「維修檢修」範圍。</p>
                </div>
                <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-850">
                  <h3 className="font-bold text-white mb-2 text-lg">Q：可以先 LINE 傳照片確認嗎？</h3>
                  <p className="text-slate-200 text-base sm:text-lg leading-[1.8]">答：可以。建議將室內機的正面外觀、掀開面板後的濾網狀況、出風口內部鰭片或風鼓的特寫照片，以及室外機的安裝環境拍照上傳至我們的官方 LINE，技師可協助進行初步的機型確認與施作評估。</p>
                </div>
              </div>
            </section>
