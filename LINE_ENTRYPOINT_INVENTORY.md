...
- `line_contact_attempt` contains only event, contact channel/method, page path, event source, bounded `service_type`, `prepare_status`, `handoff_type`, and timestamp. It never emits `lead_id`, HY token, request_id, gclid/gbraid/wbraid, session_id, raw LINE user ID, form PII, or message text. The website dataLayer contract is Production-live; GTM forwarding of the three new dimensions remains provider-publish gated until same-source GTM readback confirms the published container.
