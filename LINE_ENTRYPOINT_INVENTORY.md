...
- `line_contact_attempt` contains only event, contact channel/method, current page path, event source, bounded `service_type`, `prepare_status`, `handoff_type`, attribution `landing_path` pathname, optional numeric `campaign_id` derived only from existing `utm_id`, and timestamp. It never emits `lead_id`, HY token, request_id, gclid/gbraid/wbraid, session_id, raw LINE user ID, form PII, or message text. Query strings are stripped from `landing_path`; `campaign_id` is never inferred from click IDs. GTM forwarding of the bounded custom dimensions remains provider-publish gated until same-source GTM readback confirms the published container.
