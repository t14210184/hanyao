...
const touch = ({ gclid = null, gbraid = null, wbraid = null, utm = {} } = {}) => ({
...
try {
  ...
  const directReadback = JSON.parse(
    runWrangler([
      ...
      "--json",
      "--command",
      "SELECT session_id, first_gclid, first_gbraid, first_wbraid, first_utm_source, first_landing_path, first_referrer_origin, last_gclid, last_landing_path, server_created_at, server_updated_at, expires_at FROM attribution_sessions ORDER BY session_id; SELECT lead_token, request_id, session_id, channel, status FROM lead_tokens ORDER BY request_id;",
    ])
  );
